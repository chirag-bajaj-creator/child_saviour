...
export default function LoginModal({ isOpen, onClose, role = 'controller', schoolId = null }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/login', {
        email,
        password,
        school_id: schoolId || '507f1f77bcf86cd799439011',
      })

      // Validate role
      if (response.data.user.role !== role) {
        setError(`Invalid credentials. Only ${role === 'controller' ? 'principals' : role + 's'} can login here. You are a ${response.data.user.role}.`)
        setPassword('')
        return
      }
